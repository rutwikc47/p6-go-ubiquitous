...
public class SunshineSyncAdapter extends AbstractThreadedSyncAdapter implements GoogleApiClient.ConnectionCallbacks,GoogleApiClient.OnConnectionFailedListener {
    ...
    public void sendData(){
        ...
        hightxt=Utility.formatTemperature(mContext,cursor.getDouble(INDEX_MAX_TEMP));
        lowtxt=Utility.formatTemperature(mContext,cursor.getDouble(INDEX_MIN_TEMP));
        int weatherId = cursor.getInt(INDEX_WEATHER_ID);

        Log.e(LOG_TAG,hightxt);

        PutDataMapRequest mapRequest=PutDataMapRequest.create("/step-counter");

        mapRequest.getDataMap().putString("high", hightxt);
        mapRequest.getDataMap().putString("low", lowtxt);
        mapRequest.getDataMap().putInt("weatherid",weatherId);
        mapRequest.getDataMap().putLong("time", new Date().getTime());
